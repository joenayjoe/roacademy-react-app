...
const UserPhotoSetting: React.FunctionComponent = () => {
  ...
  const flashErrorHolder = uploadError.length ? (
    <Alert
      title="Following errors prevent from uploading : "
      errors={uploadError}
  ...
  const getImageUploadInput = () => {
    ...
    if (selectedFile == null || isCropped) {
      let fileLabel =
        selectedFile == null ? "Choose a photo" : selectedFile.name;
      imageInput = (
        <div className="input-group mb-3">
          <div className="custom-file">
            <input
              ...
              multiple={false}
              onChange={(e) => handleProfilePhotoInputOnChange(e.target.files)}
            />
  ...
  } else if (authContext.currentUser && authContext.currentUser.imageUrl) {
    ...
    <UserSettingContainer>
      <div className="user-profile-form-wrapper">
        ...
        <div className="user-profile-footer">
          <button className="btn btn-primary mt-2" onClick={uploadProfilePhoto}>
            Save
          </button>
        </div>
      </div>
